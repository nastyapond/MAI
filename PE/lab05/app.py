...
def create_user(user: UserCreate, db: Session = Depends(get_db)):
    ...
    db.commit()
    db.refresh(db_user)
    redis_client.set(f"user:{user.username}", json.dumps({"username": user.username}), ex=60)
...
def get_user_from_cache_or_db(username: str, db: Session):
    ...
    if user:
        ...
        redis_client.set(f"user:{username}", json.dumps(user_data), ex=60)
        return user_data
    return None
...
def get_user_info(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    user_data = get_user_from_cache_or_db(current_user.username, db)
    if not user_data:
        raise HTTPException(status_code=404, detail="User not found")
