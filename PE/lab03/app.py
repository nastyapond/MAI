...
def login(form_data: UserCreate, db: Session = Depends(get_db)):
    ...
    if not user or not verify_password(form_data.password, user.hashed_password):  # Проверяем хэш пароля
...
def send_message(username: str, msg: MessageCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    new_message = Message(
        sender=current_user.username,  # Используем current_user.username
...
def get_messages(username: str, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    if current_user.username != username and current_user.username != "admin":  # Используем current_user.username
...
def delete_message(username: str, message_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    if message.sender != current_user.username and current_user.username != "admin":  # Используем current_user.username
